...
<template>
  <footer class="dbzu14">
    <section class="jla703">
      <div class="g2zege">
        <div class="xap1v2">
          <div class="p7v3cs">
            <img src="../assets/logo.png" alt="logo" class="sr59zz">
        ...
        <div class="pa3v2u">
          ...
          <form @submit.prevent="validation">
            <div space-y1>
              <div>
                <input v-model="form.username" type="text" placeholder="نام *" class="q1hwzp">
                <XyzTransition xyz="fade up">
                  <p v-if="form.usernameErr" class="w20xet ">{{ form.usernameErr }}</p>
                </XyzTransition>
              </div>
              <div>
                <input v-model="form.email" type="email" placeholder="پست الکترونیک *" class="q1hwzp">
                <XyzTransition xyz="fade up">
                  <p v-if="form.emailErr" class="w20xet">{{ form.emailErr }}</p>
                </XyzTransition>
              </div>
              <div>
                <input v-model="form.message" type="text" placeholder="موضوع" class="q1hwzp">
              </div>
              <div>
                <textarea v-model="form.content" cols="25" rows="3" placeholder="پیغام *" class="q1hwzp"></textarea>
                <XyzTransition xyz="fade up">
                  <p v-if="form.contentErr" class="w20xet !mt0">{{ form.contentErr }}</p>
                </XyzTransition>
              </div>
            </div>
    ...
    <section class="l62nxb">
      <p class="g18nft">© ۲۰۲۲ تمامی حقوق برای این قالب محفوظ است​​​​​​​.</p>
    </section>
  </footer>
</template>
...
.i87qry>:not([hidden])~:not([hidden]) {
  --un-space-y-reverse: 0;
  margin-top: calc(0.50rem * calc(1 - var(--un-space-y-reverse)));
  margin-bottom: calc(0.50rem * var(--un-space-y-reverse));
}
...
.e72srp {
  ...
  text-decoration-line: underline;
  -webkit-text-decoration-color: rgba(156, 163, 175, var(--un-line-opacity));
  --un-line-opacity: 1;
  text-decoration-color: rgba(156, 163, 175, var(--un-line-opacity));
}
